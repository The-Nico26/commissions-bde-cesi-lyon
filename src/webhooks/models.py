...
class Webhook(models.Model):
    ...
    def trigger(self, force=False, fail_silently=True, **kwargs):
        ...
        if self.type == "discord":
            ...
            self.raise_error("Unimplemented webhook type {}".format(self.type), fail_silently)

    def trigger_discord(self, fail_silently=True, commission=None, commission_url="", commission_logo_url=""):
        message = ""
        embeds = []

        if self.trigger == "commission-create":
            ...
            if commission.treasurer is not None:
                embed["fields"] += {
                    "name": "Suppléant",
            ...
            self.raise_error("Unimplemented webhook discord trigger {}".format(self.type), fail_silently)
            return
        ...
        webhookr = requests.post(self.url, json=payload)
